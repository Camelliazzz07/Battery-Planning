...
def total(fees):
    return (
        sum(fees[k] for k in ("plan_cost", "up_cost", "down_penalty", "emergency_cost"))
        - fees["refund"]
    )
...
class Q3Tests(unittest.TestCase):
    def test_downward_settlement_and_repeated_revision(self):
        p, q0, r = np.ones(1), np.array([100.0]), np.zeros(1)
        self.assertAlmostEqual(
            total(settlement(p, q0, np.array([80.0]), r, "no_refund"))[0], 110
        )
        self.assertAlmostEqual(
            total(settlement(p, q0, np.array([80.0]), r, "refund"))[0], 90
        )
        # 100 -> 150 -> 120 -> 130: final cost 100+1.5*30, not sum of deviations.
        self.assertAlmostEqual(
            total(settlement(p, q0, np.array([130.0]), r, "no_refund"))[0], 145
        )
    ...
    def test_forecast_release_and_actual_data_causality(self):
        ...
        load, pv = np.full((10, T), 3000.0), np.full((10, T), 600.0)
        forecasts = {(d, h): np.full(24, 1200.0) for d in dates for h in (0, 6, 12, 18)}
        data = Data(dates, np.ones(T), np.ones(T) * 3000, load, pv, forecasts)
        cfg = Config(load_bias_weight=0.5)
        ...
        data.forecasts[(dates[7], 18)] = np.arange(1, 25) * 100.0
        self.assertAlmostEqual(forecast_at(data, 7, 18, Config())[1][-1] * 6, 600.0)

    def test_reserve_subtracts_actual_margin_before_accumulation(self):
        history = np.tile([10.0, -30.0, 50.0], (10, 1))
        cfg = Config(reserve_horizon=3)
        margin, reserve = risk_profile(history, np.zeros(3), np.zeros(3), 0.8, cfg)
