...
class ModelConfig:
    ...
    down_reserve_penalty: float = 0.02
    terminal_soc_target_kwh: float = E_INITIAL  # 仅baseline/forecast_only与1月预热使用
...
def legacy_build_risk_profiles(
...
def legacy_solve_day_ahead_milp(
    price: np.ndarray,
    forecast_load_kwh: np.ndarray,
    forecast_pv_kwh: np.ndarray,
    start_soc_kwh: float,
...
def revised_risk_profiles(history, forecast_load, forecast_pv, config):
    ...
    """
    if len(history) < config.min_residual_days:
        # 通常只发生在1月预热；明确为假设，不把它声称为已校准区间。
        scale = 0.05 * forecast_load + 0.08 * forecast_pv
    ...
    up = np.maximum(0.0, np.quantile(remaining, config.reserve_quantile, axis=0))
    down = np.maximum(0.0, np.quantile(-remaining, config.reserve_quantile, axis=0))
    remaining_blocks = blocks - np.tile(margin, 2)
...
def solve_two_day_milp(price, load, pv, next_load, next_pv, start_soc, risk, config,
    ...
    仍属软备用代理优化，不含情景紧急费期望或CVaR。
    """
    n = 2 * T
    groups = ["q", "c", "d", "e", "w", "su", "sd", "z", "pu", "pd"]
    offsets = {name: i * n for i, name in enumerate(groups)}
    nv = len(groups) * n
    prices = np.tile(price, 2)
    margin = np.tile(risk["plan_margin"], 2)
    ru = np.tile(risk["up_instant"], 2)
    rd = np.tile(risk["down_instant"], 2)
    eu = np.tile(risk["up_cumulative"], 2) / ETA_D
...
def run_model(args: argparse.Namespace) -> dict[str, Path]:
    config = ModelConfig(
        ...
        down_reserve_penalty=args.down_reserve_penalty,
    ...
    if not 1 <= config.reserve_horizon_intervals <= T:
        raise ValueError("备用观察窗必须为1至144个10分钟时段。")
    ...
    window_final_soc = np.full(n_days, np.nan)
    ...
    for day_index, date in enumerate(dates):
        # 统一1月预热，保证2月1日期初SOC相同，且没有跨日重置。
        variant = "baseline" if date < OUTPUT_START else config.variant
        forecast_load_kw, load_weights = adaptive_historical_forecast(
            load_kw, dates, day_index, prior_load_kw
        )
        if variant in {"forecast_only", "revised"}:
            forecast_load_kw, load_weights = seasonal_load_forecast(
                load_kw, day_index, prior_load_kw
            )
        ...
        if date == OUTPUT_START and variant in {"forecast_only", "revised"}:
            # 切换预测器时重建其1月滚动残差；每个历史预测仍只用当时的过去。
            # 不改变1月已经模拟过的物理运行，也不使用2月实际值校准。
            residual_history = []
        ...
        if variant == "revised":
            ...
            risk = legacy_build_risk_profiles(
                residual_history, forecast_load_kwh, forecast_pv_kwh, config
            )
        if variant in {"soc_only", "revised"}:
            ...
            plan = legacy_solve_day_ahead_milp(
                price, forecast_load_kwh, forecast_pv_kwh, current_soc, risk, config
            )
        # 预测区间与调度备用分开：旧版区间仅用于复现对照。
        arrays["interval_lower"][day_index] = risk.get(
            "interval_lower", -risk["down_instant"]
        )
        arrays["interval_upper"][day_index] = risk.get(
            "interval_upper", risk["plan_margin"] + risk["up_instant"]
        )
        arrays["power_shortfall_up"][day_index] = plan.get("pu", np.zeros(T))
        arrays["power_shortfall_down"][day_index] = plan.get("pd", np.zeros(T))
        ...
        terminal_deviation[day_index] = float(plan.get("terminal_deviation", 0.0))
        ...
        frame = pd.DataFrame(
            {
                ...
                "正向功率备用缺口_kWh": arrays["power_shortfall_up"][day_index],
                "反向功率备用缺口_kWh": arrays["power_shortfall_down"][day_index],
            }
        )
        ...
        daily_rows.append(
            {
                ...
                "48小时窗口末SOC_kWh": float(window_final_soc[day_index]),
                "总剩余电量_kWh": float(arrays["spill"][day_index].sum()),
            }
        )
