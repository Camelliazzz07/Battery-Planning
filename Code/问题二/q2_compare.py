...
在 VS Code 中可直接运行。已有完整输出时可加 --skip-run。
...
def checks(ts, daily, events, path, variant):
    ...
    check(
        "334天且日期完整",
        int(
            not pd.DatetimeIndex(daily["日期"]).equals(
                pd.date_range("2025-02-01", "2025-12-31")
            )
        ),
        0,
    )
    ...
    check(
        "序号自然日排序",
        absmax(ts["区间编号"].to_numpy() - np.tile(np.arange(1, 145), 334)),
        0,
    )
    for prefix in ["计划", "实际"]:
        c, d, e = [
            ts[prefix + x + "_kWh"].to_numpy() for x in ["充电量", "放电量", "SOC"]
        ]
        if prefix == "实际":
    ...
    check(
        "跨日SOC连续",
        absmax(
            daily["期初SOC_kWh"].to_numpy()[1:] - daily["期末SOC_kWh"].to_numpy()[:-1]
        ),
    )
    check("计划费按计划量结算", abs(np.dot(q, price) - daily["计划购电费_元"].sum()))
    check(
        "紧急费按5倍电价",
        abs(np.dot(emergency, 5 * price) - daily["紧急购电费_元"].sum()),
    )
    ...
    check(
        "result2工作表",
        int(wb.sheetnames != ["计划购电量", "充放电量", "紧急购电量"]),
        0,
    )
    ...
    check(
        "144列首尾映射",
        absmax(np.asarray([row[1:145] for row in plan[1:]], float) - expected_q),
    )
    check(
        "模板每日日期",
        sum(pd.Timestamp(row[0]) != day for row, day in zip(plan[1:], daily["日期"])),
        0,
    )
    check(
        "模板计划总电量",
        absmax(np.array([row[145] for row in plan[1:]]) - qm.sum(axis=1)),
    )
    check(
        "模板计划费用",
        absmax(
            np.array([row[146] for row in plan[1:]]) - daily["计划购电费_元"].to_numpy()
        ),
    )
    ...
    check(
        "模板0点SOC",
        absmax(
            np.array([r[5] for r in storage[::6]]) - daily["期初SOC_kWh"].to_numpy()
        ),
    )
    check(
        "模板24点SOC",
        absmax(
            np.array([r[5] for r in storage[1::6]]) - daily["期末SOC_kWh"].to_numpy()
        ),
    )
    event_rows = list(wb["紧急购电量"].iter_rows(values_only=True))[1:]
    check(
        "模板紧急量", abs(sum(float(r[2] or 0) for r in event_rows) - emergency.sum())
    )
    wb.close()
    return records
...
def logic_tests():
    ...
    np.testing.assert_allclose(r["interval_upper"], np.quantile(errors, 0.95, axis=0))
    np.testing.assert_allclose(r["interval_lower"], np.quantile(errors, 0.05, axis=0))


def main():
    ap = argparse.ArgumentParser(description=__doc__)
    ap.add_argument("--attachment1", type=Path, default=ATTACHMENT_DIR / "附件1.xlsx")
    ap.add_argument("--attachment2", type=Path, default=ATTACHMENT_DIR / "附件2.xlsx")
    ap.add_argument("--template", type=Path, default=TEMPLATE_DIR / "result2.xlsx")
    ap.add_argument("--runs-dir", type=Path, default=AUXILIARY_DIR / "方案对照")
    ap.add_argument("--output-dir", type=Path, default=AUXILIARY_DIR / "对照报告")
    ap.add_argument("--skip-run", action="store_true")
    args = ap.parse_args()
    args.output_dir.mkdir(parents=True, exist_ok=True)
    if not args.skip_run:
        for variant in VARIANTS:
            result_path = args.runs_dir / variant / "result2.xlsx"
            cmd = [
                sys.executable,
                str(Path(__file__).with_name("q2_run_model.py")),
                "--attachment1",
                str(args.attachment1),
                "--attachment2",
                str(args.attachment2),
                "--template",
                str(args.template),
                "--variant",
                variant,
                "--output-dir",
                str(args.runs_dir / variant),
                "--result",
                str(result_path),
            ]
    ...
    for variant in VARIANTS:
        ...
        validations.extend(checks(ts, daily, events, folder / "result2.xlsx", variant))
        covered = ts["实际净负荷_kWh"].between(
            ts["净负荷预测下界_kWh"], ts["净负荷预测上界_kWh"]
        )
        rows.append(
            {
                "方案": variant,
    ...
    result["保守扣除期末差异后节约_元"] = result[
        "相对原模型节约_元"
    ] - 0.9 * 5 * pmax * np.abs(result["实际期末SOC_kWh"] - base["实际期末SOC_kWh"])
    report = pd.DataFrame(validations)
    report.to_csv(
        args.output_dir / "q2_validation.csv", index=False, encoding="utf-8-sig"
    )
    if not report["通过"].all():
        print(report.loc[~report["通过"]].to_string(index=False))
        raise RuntimeError("存在物理或结果表检查失败，不能交付为通过结果")
    result.to_csv(
        args.output_dir / "q2_cost_comparison.csv", index=False, encoding="utf-8-sig"
    )
    ...
    ax.legend()
    fig.tight_layout()
    fig.savefig(args.output_dir / "01_cost_comparison.png", dpi=160)
    plt.close(fig)
    ...
    fig.tight_layout()
    fig.savefig(args.output_dir / "02_cumulative_savings.png", dpi=160)
    plt.close(fig)
    fig, axs = plt.subplots(2, 1, figsize=(11, 7), sharex=True)
    for i, variant in enumerate(["baseline", "revised"]):
        d = all_daily[variant]
        axs[0].plot(d["日期"], d["计划日末SOC_kWh"], label=LABELS[variant], alpha=0.8)
        axs[1].plot(d["日期"], d["期末SOC_kWh"], label=LABELS[variant], alpha=0.8)
    for ax, label in zip(
    ...
    fig.tight_layout()
    fig.savefig(args.output_dir / "03_day_end_soc.png", dpi=160)
    plt.close(fig)
    ...
    fig.tight_layout()
    fig.savefig(args.output_dir / "04_emergency_and_surplus.png", dpi=160)
    plt.close(fig)
    rev = result.loc[result["方案"] == "revised"].iloc[0]
    text = [
        "# 问题二新版对照结果",
        "",
        "评价期：2025-02-01至2025-12-31，共334天。",
    ...
    for _, r in result.iterrows():
        text.append(
            f"| {r['方案']} | {r['计划购电费_元']:,.2f} | {r['紧急购电费_元']:,.2f} | {r['总购电费_元']:,.2f} | {r['实际期末SOC_kWh']:,.2f} |"
        )
    text.extend(
        [
            "",
            f"完整版相对原模型节约 {rev['相对原模型节约_元']:,.2f} 元（{rev['相对原模型节约率']:.2%}）。",
    ...
    (args.output_dir / "q2_comparison.md").write_text(
        "\n".join(text) + "\n", encoding="utf-8"
    )
